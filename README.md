...
A functional component can do everything a class component can do, but class components handle the lifecycle a bit dfferently. Functional components are good for light, stateless applications, while class components are good for heavy, steful applications. Class components are classes extending for the base react class, needs the render() method, event handlers and helpers are treated as class methods, and the access state using this.state & this.props.

2. When does a componentDidMount function be called? What about a componentDidUpdate?

componentDidMount is called only once, after a component's intial render() method is invoked.
componentDidUpdate is called anytime a component updates/changes props or state.

3. Define stateful logic.

Logic built into a component, specifically dealing with state.

4. What are the three step of creating a successful test? What is done in each phase?

1- Arrange: render/set up components to view.
2- Act: simulate a user event or action.
3- Assert: expect results to be as expected based on actions.
